...
IF OBJECT_ID ('dbo.Instruments', 'U') IS NOT NULL
...
CREATE TABLE dbo.Users (
    ...
    cash_balance DECIMAL(18, 4) NOT NULL DEFAULT 100000.00,
...
CREATE NONCLUSTERED INDEX IX_Prices_Instrument_Time ON dbo.Prices (
    instrument_id,
    recorded_at DESC
);
GO
...
CREATE TABLE dbo.Orders (
    ...
    status NVARCHAR (12) NOT NULL DEFAULT 'PENDING' CHECK (
        status IN (
            'PENDING',
            'EXECUTED',
            'CANCELLED',
...
CREATE NONCLUSTERED INDEX IX_Orders_User ON dbo.Orders (user_id, placed_at DESC);
GO

CREATE NONCLUSTERED INDEX IX_Orders_Status ON dbo.Orders (status, placed_at DESC);
GO
